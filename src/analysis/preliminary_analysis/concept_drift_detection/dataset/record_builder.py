"""レコード生成（毎日 0 時グリッド × アクティブ集合。§2.2, §2.4）。
...
ラベル = Δ以内レビューの2値（§2.3）。未確定（窓が観測末尾超）のレコードは作らない。
"""
...
from src.analysis.preliminary_analysis.concept_drift_detection.labeling import label_builder
...
class Record:
    ...
    features: list[float]
    labels: dict
    bin: int | None = field(default=None)  # binning で付与（§2.5）
...
def build_records(changes: list[dict], project: str, pool_start: datetime, cycle_end: datetime,
    ...
    特徴量は fast_index（事前集計）と src.features で計算（feature_builder）。
    """
