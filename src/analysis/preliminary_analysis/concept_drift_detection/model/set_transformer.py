"""集合Transformer（自己注意）＋転移学習の本体（design.md §5, §6）。

構成：
- Scaler       : 特徴の標準化（事前学習データで fit → 全体に適用。§7）
- SetEncoder   : 位置エンコなしの自己注意エンコーダ（置換不変・可変長）。各 Change に文脈込み表現を出す。§5.2
- Head         : 予測ヘッド。linear probe（線形1層）／small MLP を選択。§5.3
- pretrain     : 教師あり事前学習（エンコーダ＋仮ヘッドで「Δ以内レビュー」を予測）→ エンコーダ凍結。§6.1
- embed_sets   : 凍結エンコーダで各集合の per-item 埋め込みを 1 回計算（キャッシュ）。§6.4
- train_probe  : キャッシュ済み埋め込みの上でヘッドだけ学習（linear probing）。§6.2
- predict      : ヘッドで per-Change 予測（正例確率）を返す。

「仮ヘッド（汎用ヘッド）」は評価に使うため破棄せず返す（§8.3 の汎用ヘッド vs probe 比較）。
"""
...
from src.analysis.preliminary_analysis.concept_drift_detection.dataset.set_builder import TSet
from src.analysis.preliminary_analysis.concept_drift_detection.features.feature_builder import FEATURE_NAMES
...
# ── エンコーダ（自己注意・位置エンコなし＝集合として扱う。§5.2） ──────────
class SetEncoder(nn.Module):
    def __init__(self, d_model: int, n_layers: int, n_heads: int, ffn_dim: int, dropout: float):
        ...
        # 注：位置エンコーディングは足さない → 置換不変（集合として扱う。§5.2, design.md）
...
# ── ヘッド（linear probe / small MLP。§5.3） ─────────────────────
...
def pretrain(pretrain_sets: list[TSet], scaler: Scaler, seed: int, device: torch.device):
    ...
    仮ヘッド（＝汎用ヘッド）は §8.3 の比較評価に使うため破棄せず返す。
    """
...
# ── linear probing（キャッシュ埋め込みの上でヘッドだけ学習。§6.2） ──────────
