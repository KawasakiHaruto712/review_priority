"""要約表（主成果物。design.md §11.1）。

行＝窓長（2M/1M/2w/1w/3d/1d）、列＝バージョン、セル＝指標値（＋小さく欠測量）、右端 Ave＝バージョン横断平均。
metrics.csv から生成（再実行不要）。seed は中央値集約 → 評価日で平均。
"""
from __future__ import annotations

from pathlib import Path

import matplotlib
import numpy as np
import pandas as pd

matplotlib.use("Agg")
import matplotlib.pyplot as plt  # noqa: E402

from src.analysis.preliminary_analysis.lookback_window.io import result_io  # noqa: E402
from src.analysis.preliminary_analysis.lookback_window.utils import constants  # noqa: E402
from src.analysis.preliminary_analysis.lookback_window.visualization._fonts import configure_japanese_font  # noqa: E402

configure_japanese_font()


def _window_labels_desc() -> list[str]:
    """窓長を大きい順（2M→1d）にラベル化。"""
    return [constants.WINDOW_LABELS.get(w, f"{w}d") for w in sorted(constants.WINDOWS_DAYS, reverse=True)]


def _cell_summary(mdf: pd.DataFrame, metric: str, window_label: str):
    """(値, 欠測日数, 総日数)。seed 中央値 → 評価日平均。欠測＝その日が全 seed で NaN。"""
    sub = mdf[mdf["window"] == window_label]
    if sub.empty:
        return np.nan, 0, 0
    per_day = sub.groupby("day")[metric].median()
    total = int(per_day.shape[0])
    valid = per_day.dropna()
    miss = total - int(valid.shape[0])
    val = float(valid.mean()) if not valid.empty else np.nan
    return val, miss, total


def build_summary(project: str, versions: list[str], metric: str):
    """(val_df, miss_df, total_df) を返す。index=窓長(desc), columns=versions(+'Ave')。"""
    rows = _window_labels_desc()
    val = pd.DataFrame(index=rows, columns=versions, dtype=float)
    miss = pd.DataFrame(index=rows, columns=versions, dtype=float)
    total = pd.DataFrame(index=rows, columns=versions, dtype=float)
    for v in versions:
        try:
            mdf = result_io.load_metrics(project, v)
        except FileNotFoundError:
            continue
        for wl in rows:
            val_, miss_, total_ = _cell_summary(mdf, metric, wl)
            val.loc[wl, v] = val_
            miss.loc[wl, v] = miss_
            total.loc[wl, v] = total_
    val["Ave"] = val[versions].mean(axis=1, skipna=True)
    return val, miss, total


def render_table(val: pd.DataFrame, miss: pd.DataFrame, total: pd.DataFrame,
                 metric: str, out_path: Path) -> None:
    """要約表を PNG で描く（セル：値＋小さく欠測量）。"""
    out_path = Path(out_path)
    out_path.parent.mkdir(parents=True, exist_ok=True)
    cols = list(val.columns)                 # versions + 'Ave'
    rows = list(val.index)
    cell_text = []
    for wl in rows:
        line = []
        for c in cols:
            v = val.loc[wl, c]
            vtxt = "-" if pd.isna(v) else f"{v:.3f}"
            if c == "Ave":
                line.append(vtxt)            # Ave は欠測併記なし
            else:
                m = miss.loc[wl, c]
                t = total.loc[wl, c]
                sub = "" if pd.isna(m) or pd.isna(t) or t == 0 else f"\n欠測 {int(m)}/{int(t)}"
                line.append(vtxt + sub)
        cell_text.append(line)

    fig, ax = plt.subplots(figsize=(1.6 * len(cols) + 1, 0.9 * len(rows) + 1))
    ax.axis("off")
    ax.set_title(f"{metric}：窓長 × バージョン（右端 Ave＝バージョン横断平均）", fontsize=11)
    tbl = ax.table(cellText=cell_text, rowLabels=rows, colLabels=cols,
                   cellLoc="center", rowLoc="center", loc="center")
    tbl.auto_set_font_size(False)
    tbl.set_fontsize(9)
    tbl.scale(1.0, 1.8)
    fig.tight_layout()
    fig.savefig(out_path, dpi=constants.PLOT_DPI, bbox_inches="tight")
    plt.close(fig)


def build_and_save(project: str, versions: list[str], metric: str, out_dir: Path) -> None:
    """要約表を作って CSV＋PNG で保存。"""
    out_dir = Path(out_dir)
    out_dir.mkdir(parents=True, exist_ok=True)
    val, miss, total = build_summary(project, versions, metric)
    val.to_csv(out_dir / f"summary_{metric}.csv")
    miss.to_csv(out_dir / f"summary_{metric}_missing.csv")
    render_table(val, miss, total, metric, out_dir / f"summary_{metric}.png")
